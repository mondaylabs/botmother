...
3.	Создайте своего бота помощью `@BotFather`. Скопируйте токен созданного бота и задайте его в переменной `BOT_TOKEN` в том же `settings.py`: 
```python
...
* `run` – Обязательный аргумент: `function`. Запускает указанную функцию. 
* `new_photo` – Обязательный аргумент: `function`. Перенапрявляет в функцию и при добавлении группового фото.

Все функции принимают необязательные аргументы, как: `last_action=None, chat_type=None, edited=False, reply_type=None`
...
Функция `redirect` перенаправит на другую функцию после выполнения предыдущей. Обязательно пропишите ее в качестве аргумента в вашей функции. Далее укажите в redirect в качестве аргумента функцию, на которую вы хотите перейти.

При написании хендлеров, не стоит забывать о обязательных атрибутах, без которых ваша функция может не работать:

* `message` – обязательный атрибут, нужен, например для сохранения присланного пользователем текста.
* `chat` – самый главный атрибут, нужен для взаимодействия с клиентом. В других библиотеках зачастую используется атрибут `Bot`.
* `redirect` – опять же полезен при перенаправлении на другую функция при завершении первой.
...
`AbstractChat` уже имеет следующие переменные: `chat_id, type, username, first_name, last_action, data, last_activity`. 
...
```python
chat.send_message('Choose the language: ', reply_markup=language())
```
...
```
Клавиатура может принимать аргумент `reply_markup=True`, что растянет клавиатуру по ширине экрана (одна кнопка будет занимать одну строку). Соответственно, если вы хотите в одной строке разместить несколько кнопок, то вам следует в списке прописать столько же словарей: 
```python
def func_name ():
    return {
        'keyboard_name': [
            [{'text': '<your_text1>'}, {'text': '<your_text2>'}, 
            {'text': '<your_text3>'}{'text': '<your_text3>'}],
            [{'text': '<your_text>'}]
        ],
        'resize_keyboard': True,
    }
```
Также, можно скрыть клавиатуру после одного касания добавлением аргумента `one_time_keyboard= True`. Но не следует прописывать этот аргумент, в случаях, когда одна клавиатура заменяется другой.
```python
def func_name():
    return {
        keyboard_name: [
...
```
За дополнительной информацией вы можете перейти по [ссылке](https://core.telegram.org/) 
Немаловажный момент – это указание о существовании вашей клавиатуры в `models.py`. Текст вашей клавиатуры должен совпадать в `models.py`. Например:
...
class Chat(AbstractChat):
    ...
    language = models.CharField(max_length=255, default=EN, choices=LANGUAGES)

    class Meta(AbstractChat.Meta):
        db_table = '<your_app_name>_chats'
```
